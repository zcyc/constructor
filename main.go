...
func main() {
	...
	output := *outputFile
	if output == "" {
		output = defaultOutputFile(sourceFile, *typeName)
	}
...
func defaultOutputFile(sourceFile, typeName string) string {
	dir := filepath.Dir(sourceFile)
	base := strings.ToLower(typeName)
	for suffix := 1; ; suffix++ {
		name := base + "_gen.go"
		if suffix > 1 {
			name = fmt.Sprintf("%s%d_gen.go", base, suffix)
		}
		filename := filepath.Join(dir, name)
		if _, err := os.Lstat(filename); err != nil {
			return filename
		}
		if generatedForType(filename, typeName) {
			return filename
		}
	}
}
...
func generatedForType(filename, typeName string) bool {
	...
	if err != nil || !strings.Contains(string(content), "Code generated by constructor. DO NOT EDIT.") {
		return false
	}
	contentString := string(content)
	return strings.Contains(contentString, "func New"+typeName+"(") ||
		strings.Contains(contentString, "func New"+typeName+"Builder") ||
		strings.Contains(contentString, "type "+typeName+"Option")
}
